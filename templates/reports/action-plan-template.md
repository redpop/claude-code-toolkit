...
- [ ] **Input Sanitization** (3h)
  - Command: `/global:secure . --focus="xss,sanitization" --auto-fix`
  - Files: `src/components/*`, `src/utils/*`
  - Impact: Prevents XSS attacks in all user inputs
  - Dependencies: None
  
- [ ] **JSON Validation** (2h)
  - Command: `/global:secure . --focus="validation" --library="zod"`
...
- [ ] **Remove Sensitive Logs** (1h)
  - Command: `/global:secure . --remove-sensitive-logs --production`
  - Files: All `*.ts` and `*.tsx` files
  - Impact: Prevents data leaks in production
  - Dependencies: None
  
- [ ] **Add CSP Headers** (2h)
  - Command: `/global:secure . --add-csp-headers --strict`
...
- [ ] **Remove Code Duplication** (2h)
  - Command: `/global:improve . --duplicates --file="src/utils/pdfExport.ts" --extract-common`
  - Lines saved: 140
  - Impact: 50% maintenance effort reduction
  - Dependencies: None

- [ ] **Batch Database Operations** (4h)
  - Command: `/global:improve . --performance --auto-fix --focus="database"`
...
- [ ] **Increase Test Coverage** (8h)
  - Command: `/global:create tests --coverage-target=60 --focus="critical"`
  - Current: 30% → Target: 60%
  - Priority modules: `pdfExport`, `dataImport`, `storageAdapter`
  - Dependencies: Jest configuration
  
- [ ] **Add Type Safety** (4h)
  - Command: `/global:improve . --types --strict --no-any`
  - Remove: 15 `any` types
...
- [ ] **Create Security Baseline**
  - Command: `/global:secure . --baseline --save`
  - When: Before starting any fixes
  - Purpose: Measure improvement
  
- [ ] **Weekly Progress Review**
  - Command: `/global:understand . --deep --compare-baseline`
...
/global:secure . --compliance --check

# After quick wins
/global:understand . --quick --compare-baseline

# Final validation
/global:understand . --deep --compare-baseline
```

### 4. Generate Completion Report

```bash
/global:ship . --completion-report --action-plan=this-file.md
```
...
---
*Generated by `/global:understand . --analyze-report --generate-action-plan`*
